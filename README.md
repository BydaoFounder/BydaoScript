...
Сам язык, его компилятор в байткод и виртуальная машина - в процессе разработки. Все средства еще очень далки от реального использования.
...
var i = 0
var t1 = Sys.time()
while i < 1000000 next i = i + 1 {  // операторв части next выполняется в конце цикла
    sum = sum + i
}
t1 = Sys.time() - t1
...
sum = 0
t = Sys.time()
iter Int.range( 0, 1000000 ) as it {
    sum = sum + it.value
}
t = Sys.time() - t
Sys.outln( 'time: ' + t + ' msec, sum = ' + sum )
...
Идеи относительно ролевого полиморфизма изложены в папке "/doc".
