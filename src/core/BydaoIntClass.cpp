...
MetaData*   BydaoIntClass::metaData() {
    ...
    if ( ! metaData ) {
        ...
        metaData
            ...
            // Методы класса
            ->appendType( "range",  FuncMetaData(0,"IntRange", FMD_IMMUTABLE)
                                  << FuncArgMetaData("from","Int")
                                  << FuncArgMetaData("to","Int")
                     )
            .appendType( "max",     FuncMetaData(1,"Int", FMD_IMMUTABLE)
                               << FuncArgMetaData("arg1","Int")
                               << FuncArgMetaData("arg2","Int")
                    )
            .appendType( "min",     FuncMetaData(2,"Int", FMD_IMMUTABLE)
                               << FuncArgMetaData("arg1","Int")
                               << FuncArgMetaData("arg2","Int")
                    )
            .appendType( "parse",   FuncMetaData(3,"Int", FMD_IMMUTABLE)
                                 << FuncArgMetaData("str","String")
                    )
            .appendType( "random",  FuncMetaData(4,"Int", FMD_IMMUTABLE)
                                  << FuncArgMetaData("from","Int")
                                  << FuncArgMetaData("to","Int")
                    )
            ;
